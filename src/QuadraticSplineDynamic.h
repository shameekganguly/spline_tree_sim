...
public:
  ...
  QuadraticSplineDynamic(const QuadraticSplineKinematic *kinematic)
      : _kinematic(kinematic), _ks(6.0 * 1e6), _bs(0.50), _mass(0.1),
        _home_axis(1.0, 0.0, 0.0) {
    // nothing to do
  }
  ...
  void springForce(Eigen::Vector2d &ret_vec) {
    ...
    spring_jac(0) = cb * sa * _home_axis[0] + ca * cb * _home_axis[2];
    spring_jac(1) =
        ca * sb * _home_axis[0] - cb * _home_axis[1] - sa * sb * _home_axis[2];
    ret_vec = -stiffness() * spring_jac *
              (1.0 - ca * cb * _home_axis[0] - sb * _home_axis[1] -
               sa * sb * _home_axis[2]);
  }

...
void TreeVisual::updateGraphics() {
  cGenericObject *chai_parent = getParent();
  assert(chai_parent != nullptr);
...
void TreeVisual::branchMaterialIs(cMaterialPtr material) {
  // iterate over all branches
  for (auto itr = _kinematic->branchesItrBegin();
       itr != _kinematic->branchesItrEnd(); ++itr) {
...
void TreeVisual::fruitMaterialIs(cMaterialPtr material) {
  // iterate over all fruits
  for (auto itr = _kinematic->fruitsItrBegin();
       itr != _kinematic->fruitsItrEnd(); ++itr) {

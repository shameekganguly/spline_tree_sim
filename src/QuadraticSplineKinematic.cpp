...
double QuadraticSplineKinematic::radius(double s) const {
  if (s < (0 - _radius - 1e-10) || s > (_length + _radius + 1e-10)) {
    throw(std::runtime_error(
        "splineLocation: Passed value of s is out of bounds."));
  }
  if (s < 0.0) {
...
void QuadraticSplineKinematic::splineLocation(Eigen::Vector3d &ret_vector,
                                              double s) const {
...
void QuadraticSplineKinematic::splineOrientation(Eigen::Matrix3d &ret_matrix,
                                                 double s) const {
  // check for correct ranges
  if (s < (0 - _radius - 1e-10) || s > (_length + _radius + 1e-10)) {
    throw(std::runtime_error(
        "splineOrientation: Passed value of s is out of bounds."));
  }
  _splineOrientation(ret_matrix, s);
}

// get the local position of a point given the deformation coordinates in polar
// form
void QuadraticSplineKinematic::deformedLocation(
    Eigen::Vector3d &ret_vector, const SplinePointPolar &point) const {
  // check for correct ranges
  if (point.s < (0 - _radius - 1e-10) ||
      point.s > (_length + _radius + 1e-10)) {
    throw(std::runtime_error(
        "splineOrientation: Passed value of s is out of bounds."));
  }
  if (point.t < (0 - 1e-10) || point.t > (_radius + 1e-10)) {
    throw(std::runtime_error(
        "deformedLocation: Passed value of t is out of bounds."));
  }
  if (point.eta < (0 - 1e-10) || point.eta > (2.0 * M_PI + 1e-10)) {
    throw(std::runtime_error(
        "deformedLocation: Passed value of eta is out of bounds."));
  }
  ...
  Vector3d local_frame_pos(0.0, point.t * cos(point.eta),
                           point.t * sin(point.eta));
  ret_vector += spline_rot * local_frame_pos;
}

// get the local position of a point given the deformation coordinates in
// Cartesian form
void QuadraticSplineKinematic::deformedLocation(
    Eigen::Vector3d &ret_vector, const SplinePointCartesian &point) const {
  ...
  if (t < (0 - 1e-10) || t > (_radius + 1e-10)) {
    throw(std::runtime_error(
        "deformedLocation: Passed value of px and py are out of bounds."));
  }
...
double QuadraticSplineKinematic::splineProjectionLength(double s) const {
  return s * s * gam() / (2.0 * _length) *
         pow(sinc(s * gam() / 2.0 / _length), 2);
}

// compute linear jacobian of point on spline
void QuadraticSplineKinematic::splineLinearJacobian(Eigen::MatrixXd &ret_matrix,
                                                    double s) const {
  if (s < (0 - _radius - 1e-10) || s > (_length + _radius + 1e-10)) {
    throw(std::runtime_error(
        "splineOrientation: Passed value of s is out of bounds."));
  ...
  // py_dalp
  ret_matrix(1, 0) =
      s * s / (2.0 * _length) * gam_bracket * tb / den * dg_dalp -
      s * s * g / (2.0 * _length) * sinc2 * tb * sa * ca / pow(den, 3);

  // pz_dalp
  ret_matrix(2, 0) =
      -s * s / (2.0 * _length) * gam_bracket * sa / den * dg_dalp -
  ...
  // py_dbeta
  ret_matrix(1, 1) =
      s * s / (2.0 * _length) * gam_bracket * tb / den * dg_dbeta +
      s * s * g / (2.0 * _length) * sinc2 * sa * sa / pow(den, 3) / pow(cb, 2);

  // pz_dbeta
  ret_matrix(2, 1) =
      -s * s / (2.0 * _length) * gam_bracket * sa / den * dg_dbeta +
      s * s * g / (2.0 * _length) * sinc2 * sa * tb / pow(den, 3) / pow(cb, 2);
}

// compute linear jacobian of point not necessarily on the spline
void QuadraticSplineKinematic::splineLinearJacobian(
    Eigen::MatrixXd &ret_matrix,
  ...
  // check for correct ranges
  if (s < (0 - _radius - 1e-10) || s > (_length + _radius + 1e-10)) {
    throw(std::runtime_error("Passed value of s is out of bounds."));
  }
  if (t < (0 - 1e-10) || t > (_radius + 1e-10)) {
    throw(std::runtime_error("Passed value of px and py are out of bounds."));
  }
...
// compute partial derivative of the rotation matrix for the frame at a point
// along the spline
void QuadraticSplineKinematic::splinedRotdq(Eigen::Matrix3d &ret_dRot_dalp,
                                            Eigen::Matrix3d &ret_dRot_dbeta,
                                            double s) const {
  if (s < (0 - _radius - 1e-10) || s > (_length + _radius + 1e-10)) {
    throw(std::runtime_error(
        "splineOrientation: Passed value of s is out of bounds."));
...
// computations
void QuadraticSplineKinematic::splineTipProjectionLengthJacobian(
    Eigen::MatrixXd &ret_matrix) const {
  splineProjectionLengthJacobian(ret_matrix, _length);
}

// compute jacobian of the projection length at a given point along the spline,
// for spring force computations
void QuadraticSplineKinematic::splineProjectionLengthJacobian(
    Eigen::MatrixXd &ret_matrix, double s) const {
  // reshape to 1 x 2
...
void QuadraticSplineKinematic::splineCurvatureJacobian(
    Eigen::MatrixXd &ret_matrix) const {
  ret_matrix.setZero(1, 2);
...
// compute closest point on spline to a given point represented in the spline's
// frame NOTE: this treats the surface of the spline as generated by a constant
// radius sphere swept along the length of the spline.
void QuadraticSplineKinematic::closestPointToPoint(
    SplinePointCartesian &ret_point, double &distance_to_surface,
    Eigen::Vector3d &normal, const Eigen::Vector3d &test_point) const {
  double g = gam();
  double sa = sin(_alpha);
  double sb = sin(_beta);
  double ca = cos(_alpha);
  ...
  if (n.norm() < 1e-3) {
    ...
    if (ret_point.s > _length) {
      ret_point.s = _length;
      on_boundary = true;
    } else if (ret_point.s < 0) {
      ret_point.s = 0.0;
      on_boundary = true;
    }
  } else {
    ...
    if (fabs(g * p_proj[0]) < 1e-4 &&
        fabs(_length - g * unit_y.dot(p_proj)) < 1e-4) {
      ...
      ret_point.s =
          _length / g * atan2(g * p_proj[0], _length - g * unit_y.dot(p_proj));
...
// compute closest point on spline to a given sphere with the center represented
// in the spline's frame NOTE: this treats the surface of the spline as
// generated by a constant radius sphere swept along the length of the spline.
void QuadraticSplineKinematic::closestPointToSphere(
    SplinePointCartesian &ret_point, double &distance_to_surface,
    Eigen::Vector3d &normal, const Eigen::Vector3d &test_sphere_center,
    const double radius) const {
  // first find the point closest to the center of the sphere
  closestPointToPoint(ret_point, distance_to_surface, normal,
                      test_sphere_center);
...
void QuadraticSplineKinematic::_splineLocation(Eigen::Vector3d &ret_vector,
                                               double s) const {
  ...
  if (fabs(_alpha) < 1e-3 && fabs(_beta) < 1e-3) {
    ...
    double sp = s * s * gam() / (2.0 * _length) *
                pow(sinc(s * gam() / (2.0 * _length)), 2);
    double den = sqrt(pow(tan(_beta), 2) + pow(sin(_alpha), 2));
    // TODO: Correct sign for every formula using tan(beta), here and below
    // when _beta > PI/2 or < -PI/2
...
void QuadraticSplineKinematic::_splineOrientation(Eigen::Matrix3d &ret_matrix,
                                                  double s) const {
  if (fabs(_alpha) < 1e-3 && fabs(_beta) < 1e-3) {

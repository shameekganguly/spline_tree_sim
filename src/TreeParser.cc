...
TreeParser::TreeParser(const std::string &tree_file) : _file_name(tree_file) {
  ...
  if (NULL == tree_xml) {
    throw(std::runtime_error("No tree description found."));
  }
  // - - parse tree name
  {
    const char *name = tree_xml->Attribute("name");
    if (!name) {
      throw(std::runtime_error("Tree must have a name attribute."));
    }
  ...
  // - parse optional fixed attribute
  {
    ...
    if (!bool_string) {
      _tree.f_fixed_assigned = false;
    } else {
      _tree.f_fixed_assigned = true;
      _tree.fixed = parseBool(bool_string);
    }
  }
  ...
  if (NULL == trunk_xml) {
    throw(std::runtime_error("No trunk description found."));
  }
  _tree.trunk = parseTrunk(trunk_xml);

  // - parse branches
  for (tinyxml2::XMLElement *branch_xml = tree_xml->FirstChildElement("branch");
       branch_xml; branch_xml = branch_xml->NextSiblingElement("branch")) {
  ...
  // - parse fruits
  for (tinyxml2::XMLElement *fruit_xml = tree_xml->FirstChildElement("fruit");
       fruit_xml; fruit_xml = fruit_xml->NextSiblingElement("fruit")) {
...
// upon deletion
TreeKinematic *TreeParser::loadDescToTree() {
  // initialize tree
  ...
  if (_tree.f_fixed_assigned) {
    ret_tree->FixedIs(_tree.fixed);
  }
  ...
  while (branches_added.size() != _tree.branches.size()) {
    ...
    if (NULL != parent) {
      // insert branch
      auto branch_ptr = ret_tree->branchIs(bmap_itr->second.name,
                                           bmap_itr->second.parent.name, 0.0);
      ...
      if (bmap_itr->second.f_dynamic_assigned) {
        ...
        if (dyn_str.f_ks_assigned) {
          branch_ptr->splineDynamic()->_ks = dyn_str.ks;
        }
        if (dyn_str.f_bs_assigned) {
          branch_ptr->splineDynamic()->_bs = dyn_str.bs;
        }
...
Eigen::Vector3d TreeParser::parsePosition(const std::string &postion_string) {
  ...
  while (std::getline(ss, item, ' ')) {
    if (i > 2) {
      throw(std::runtime_error("Malformed position string."));
    }
...
Eigen::Quaterniond
TreeParser::parseOrientation(const std::string &orientation_string) {
  ...
  while (std::getline(ss, item, ' ')) {
    if (i > 3) {
      throw(std::runtime_error("Malformed orientation string."));
    }
    temp_vec[i++] = std::stod(item);
  }
  if (fabs(1.0 - temp_vec.norm()) > 1e-3) {
    throw(std::runtime_error("Quaternion is not unit norm."));
  }
  temp_vec.normalize(); // to remove round off errors
  Eigen::Quaterniond ret_qtn(temp_vec[0], temp_vec[1], temp_vec[2],
                             temp_vec[3]);
  return ret_qtn;
}

Eigen::Quaterniond
TreeParser::parseEulerXZXDeg(const std::string &orientation_string) {
  ...
  while (std::getline(ss, item, ' ')) {
    if (i > 2) {
      throw(std::runtime_error("Malformed Euler XZX string."));
    }
    temp_vec[i++] = std::stod(item);
  }
  Eigen::Quaterniond ret_qtn =
      Eigen::AngleAxisd(temp_vec[0] * M_PI / 180.0, Eigen::Vector3d::UnitX()) *
      Eigen::AngleAxisd(temp_vec[1] * M_PI / 180.0, Eigen::Vector3d::UnitZ()) *
      Eigen::AngleAxisd(temp_vec[2] * M_PI / 180.0, Eigen::Vector3d::UnitX());
  return ret_qtn;
}

TrunkString TreeParser::parseTrunk(tinyxml2::XMLElement *trunk_element) {
  TrunkString ret_trunk;
  tinyxml2::XMLElement *spline_elem =
      trunk_element->FirstChildElement("spline");
  if (NULL == spline_elem) {
...
SplineString TreeParser::parseSpline(tinyxml2::XMLElement *spline_element) {
...
DynamicString TreeParser::parseDynamic(tinyxml2::XMLElement *dynamic_element) {
  ...
  // as we only parse the X axis of the resulting frame
  {
    ...
    if (!home_str) {
      throw(std::runtime_error("Missing home axis definition."));
    }
...
BranchString TreeParser::parseBranch(tinyxml2::XMLElement *branch_element) {
  ...
  // parse name
  {
    ...
    if (!name) {
      throw(std::runtime_error("Branch must have a name attribute."));
    }
  ...
  tinyxml2::XMLElement *parent_elem =
      branch_element->FirstChildElement("parent");
  if (NULL == parent_elem) {
    throw(std::runtime_error("Branch parent not specified."));
  }
  ret_branch.parent = parseParent(parent_elem);

  // parse spline
  tinyxml2::XMLElement *spline_elem =
      branch_element->FirstChildElement("spline");
  if (NULL == spline_elem) {
    throw(std::runtime_error("Spline not specified."));
  }
  ...
  tinyxml2::XMLElement *dynamic_elem =
      branch_element->FirstChildElement("dynamic");
...
FruitString TreeParser::parseFruit(tinyxml2::XMLElement *fruit_element) {
  ...
  // parse name
  {
    ...
    if (!name) {
      throw(std::runtime_error("Fruit must have a name attribute."));
    }
  ...
  // parse radius
  {
    ...
    if (!radius_str) {
      throw(std::runtime_error("Fruit must have a radius attribute."));
    }
    ret_fruit.radius = std::stod(radius_str);
  }
  // parse parent
  tinyxml2::XMLElement *parent_elem =
      fruit_element->FirstChildElement("parent");
  if (NULL == parent_elem) {
    throw(std::runtime_error("Fruit parent not specified."));
  }
...
ParentString TreeParser::parseParent(tinyxml2::XMLElement *parent_element) {
  ParentString ret_parent;
  // parse name
  {
    const char *name = parent_element->Attribute("name");
    if (!name) {
      throw(std::runtime_error("Parent must have a name attribute."));
    }
  ...
  // parse s
  {
    ...
    if (!s_string) {
      throw(std::runtime_error("Parent must have a -s- attribute."));
    }

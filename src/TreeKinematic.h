...
public:
  // ctor
  TreeKinematic(const std::string &name,
                const std::string &trunk_branch_name = "trunk")
  ...
  // add branch
  BranchKinematic *branchIs(const std::string &name,
                            const std::string &parent_name, double s);
  ...
  BranchList::iterator branchesItrBegin() { return _branches.begin(); }
  BranchList::iterator branchesItrEnd() { return _branches.end(); }

  // get const iterators
  BranchList::const_iterator branchesItrBegin() const {
    return _branches.cbegin();
  }
  BranchList::const_iterator branchesItrEnd() const { return _branches.cend(); }
  ...
  void branchInfoInParentUpdate(const std::string &branch_name,
                                const ChildBranchInfo &new_info) {
  ...
  // add fruit
  Fruit *fruitIs(const std::string &name, const std::string &branch_name,
                 double s);
  ...
  // get non-const iterators
  FruitList::iterator fruitsItrBegin() { return _fruits.begin(); }
  FruitList::iterator fruitsItrEnd() { return _fruits.end(); }

  // get const iterators
  FruitList::const_iterator fruitsItrBegin() const { return _fruits.cbegin(); }
  FruitList::const_iterator fruitsItrEnd() const { return _fruits.cend(); }
  ...
  void fruitInfoInParentBranchUpdate(const std::string &fruit_name,
                                     const FruitInfo &new_info) {
    // get parent for branch
  ...
  // get trunk name
  const std::string &trunk() const { return _trunk->name(); }
  ...
  // get transform
  const Eigen::Affine3d &transformFromWorld() const { return _transform; }

  // Update bs for all branch elements. convenience function
  void branchBSIs(double bs);

  // Update ks for all branch elements. convenience function
  void branchKSIs(double ks);
  ...
  virtual void transformInTree(Eigen::Affine3d &ret_trans,
                               const std::string &branch_name, double s) const;

  // get transform in world frame
  virtual void transformInWorld(Eigen::Affine3d &ret_trans,
                                const std::string &branch_name, double s) const;

  // get position in tree frame
  virtual void positionInTree(Eigen::Vector3d &ret_vec,
                              const std::string &branch_name, double s) const;

  // get orientation in tree frame
  virtual void orientationInTree(Eigen::Matrix3d &ret_mat,
                                 const std::string &branch_name,
                                 double s) const;

  // get position in world frame
  virtual void positionInWorld(Eigen::Vector3d &ret_vec,
                               const std::string &branch_name, double s) const;

  // get orientation in world frame
  virtual void orientationInWorld(Eigen::Matrix3d &ret_mat,
                                  const std::string &branch_name,
                                  double s) const;
  ...
  virtual void jacobianLinear(Eigen::MatrixXd &ret_mat,
                              const std::string &branch_name,
